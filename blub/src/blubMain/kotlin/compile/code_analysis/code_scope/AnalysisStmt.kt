...
sealed class AnalysisStmt(val astStmt: Stmt) {
    ...
    data class If(val stmt: Stmt.If, val scope: CodeScope, val guard: CodeExprHandle): AnalysisStmt(stmt)
    data class For(val stmt: Stmt.For, val scope: CodeScope, val captureVars: Array<String>, val iterExprHandle: CodeExprHandle): AnalysisStmt(stmt)
    data class FunctionDecl(val stmt: Stmt.FunctionDecl, val funcInfo: TypeInfo.Fn, val scope: CodeScope): AnalysisStmt(stmt)
